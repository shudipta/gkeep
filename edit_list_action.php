<?php 
	...
	if(mysql_query("UPDATE notes SET num_info = '$num_info', title = '$title', content = '$content' WHERE id='$id_note';", $link)) {
		...
		echo '<p>Error: cannot execute \'update\' query in notes during editing list.</p>' . mysql_error($link) . '<br>';
	...
	if($dtime != null) {
		...
		if( empty( $error_list ) ) {
			...
			if ( mysql_num_rows( $result ) > 0 ) {
				if( mysql_query( "UPDATE reminders SET alarm_date = '$alarm_date' WHERE id_note = '$id_note';", $link ) ) {
					...
					include "src/head.php";
					include "src/header.php";
					echo '<p>Error: cannot execute \'update\' query in reminders during editing list.</p>' . mysql_error($link) . '<br>';
					// include "src/mainmenu.php";
					include "src/footer.php";
					include "src/bottom.php";
					// mysql_close($link);
		...
		if( mysql_query( "DELETE FROM reminders WHERE id='$id_note';", $link ) )
			...
			include "src/head.php";
			include "src/header.php";
			echo '<p>Error: cannot execute \'delete\' query in reminders during editing list.</p>' . mysql_error($link) . '<br>';
			// include "src/mainmenu.php";
			include "src/footer.php";
			include "src/bottom.php";
			// mysql_close($link);
