<?php 
	...
	if($result1 == false) {
		...
		echo '<p>Error: cannot execute note entry in \'add_note\' query</p>';
	...
	echo 'data from \'add_note\' inserted.';
	$sql2 = "UPDATE users SET num_of_note = (SELECT COUNT(*) FROM notes WHERE num_info = 0 AND id_user = '$id_user') WHERE id = '$id_user'";
	...
	if($result2 == false) {
		...
		include "src/header.php";
		include "src/mainmenu.php";
		echo '<p>Error: cannot execute \'update\' users query for \'num_of_note\' and \'num_of_list\'.</p>';
		include "src/footer.php";
		include "src/bottom.php";
		exit;
	}
	...
	if($dtime != null) {
		...
		if( empty( $error_list ) ) {
			...
			if ($result3 == false) {
				...
				include "src/header.php";
				include "src/mainmenu.php";
				echo '<p>Error: cannot execute \'insert\' query in reminders during create new note</p>';
				include "src/footer.php";
				include "src/bottom.php";
				exit;
			}
